...
export const getOrg = () => (dispatch) => {
    ...
    axios.get("http://localhost:8000/organization").then((res) => {
...
export const getDonatedData = (token) => (dispatch) => {
    dispatch({ type: GET_REQUEST });
    axios.get("http://localhost:8000/donatedData", {
        headers : {
...
export const postCategory = (data, id, token) => (dispatch) => {
    // console.log(id)
    dispatch({ type: GET_REQUEST });
    axios.patch(`http://localhost:8000/donatedData/update/${id}`, { category: data }, {
        headers: {
            "Authorization": `Bearer ${token}`
        }
    }).then((res) => {
