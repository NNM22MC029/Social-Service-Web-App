...
const Fundraise = () => {
  ...
  const fetchDonationCount = async () => {
    try {
      const res = await axios.get(`http://localhost:8000/donation/count`);
      console.log(res);
      console.log(res.data?.healtCount);
      setStudyDonation(
        res.data[0].educationCount?.length > 0
          ? res.data[0].educationCount[0].count
          : 0
      );
      setHealthdonation(
        res.data[0].healtCount?.length > 0 ? res.data[0].healtCount[0].count : 0
      );
      setFooddonation(
        res.data[0].foodCount?.length > 0 ? res.data[0].foodCount[0].count : 0
      );
      setUkrainedonation(
        res.data[0].indiaDonation?.length > 0 ? res.data[0].indiaDonation[0].count : 0
      );
    } catch (error) {
      // alert(error);
    }
