...
const Dashboard = () => {
  ...
  const { users, searchusers, qval, isError, isLoading } = useSelector(
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < users.length; i++) {
      price += recentDonors[i]?.amount;
      const donationDate = new Date(users[i].date).setHours(0, 0, 0, 0);
      if (donationDate === today) {
        totalTodayDonation += users[i].amount;
      }
      if (recentDonors[i]?.category == 'education') {
        totalstudyDonation += recentDonors[i].amount;
      } else if (recentDonors[i]?.category == 'indiadonation') {
        totalUkraineDonation += recentDonors[i]?.amount;
      }
    }
  ...
  const fetchRecentUserData = async () => {
    try {
      const res = await axios.get(`http://localhost:8000/donar/recent`);
  ...
  const fetchGraphData = async () => {
    try {
      const res = await axios.get(`http://localhost:8000/dashboard/graph`);
      console.log(res);
      setGraphData(res.data?.monthlyTotals);
    } catch (error) {
      console.log(error);
    }
  };
  const fetchCardData = async () => {
    try {
      const res = await axios.get(`http://localhost:8000/dashboard/card`);
      setDonors(res.data?.count);
      setTodayDonation(res.data?.todayDonation);
      setTotalamount(res.data?.totalDonation);
    } catch (error) {
      console.log(error);
    }
  };
